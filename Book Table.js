...
export default function BookTable() {
        ...
        setErrors((prevErrors) => {
            ...
            switch (name) {
                ...
                case 'date':
                    ...
                    break;
                case 'guests':
                    newErrors.guests = (value >= 1 && value <= 2000) ? '' : 'Guests should be between 1 and 2000.';
                    break;
                default:
                    break;
            }
    ...
    const handleSubmit = async(e) => {
        ...
        const validationErrors = {
            ...
            guests: (formData.guests >= 1 && formData.guests <= 2000) ? '' : 'Guests should be between 1 and 2000.',
        };
    ...
    return ( <
        ...
        Grid container spacing = { 2 } > {
            ['name', 'email'].map((field) => ( <
                ...
                key = { field } >
                <
                TextField required fullWidth label = { `Your ${field.charAt(0).toUpperCase() + field.slice(1)}` }
                name = { field }
            ...
            ['date', 'time', 'guests'].map((field) => ( <
                Grid item xs = { 12 }
                sm = { field === 'date' || field === 'time' ? 6 : 12 }
                key = { field } >
                <
                TextField required fullWidth label = { field.charAt(0).toUpperCase() + field.slice(1) }
                name = { field }
        ...
        color = "secondary"
        sx = {
            { width: '48%', backgroundColor: '#FF5722', color: 'white', '&:hover': { backgroundColor: '#E64A19' } }
        } >
        Book Now <
        /Button> <
        ...
        color = "primary"
        sx = {
            { width: '48%', color: '#FF5722', borderColor: '#FF5722', '&:hover': { borderColor: '#E64A19', color: '#E64A19' } }
        } >
        Back to Home <
        /Button> < /
        ButtonContainer > <
